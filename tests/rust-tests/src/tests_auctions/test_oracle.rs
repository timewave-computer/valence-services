...
fn test_update_price_manually() {
    ...
    suite.update_oracle_price(suite.pair.clone(), Some(price));
...
fn test_update_price_from_auctions() {
    ...
    assert_eq!(rounded_price.u128(), 151_u128); // 151 / 100 = 1.51
}
...
fn test_twap_less_then_3_auctions() {
    ...
    let err = suite.update_oracle_price_err(suite.pair.clone(), None);
...
fn test_config() {
    ...
    assert_eq!(
        ...
        price_oracle::state::Config {
            admin: suite.admin,
            auction_manager_addr: suite.manager_addr,
        }
    )
}
