...
fn test_min_balance() {
    ...
    let mut suite = SuiteBuilder::default()
        ...
        let old_config = suite.query_rebalancer_config(suite.get_account_addr(0)).unwrap();
    ...
    let new_config = suite.query_rebalancer_config(suite.get_account_addr(0)).unwrap();

    new_config.targets.iter().for_each(|new_target| {
        let target = old_config.targets.iter().find(|t| t.denom == new_target.denom).unwrap();
